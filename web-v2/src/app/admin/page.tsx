  ...
  return (
    <main className="mx-auto max-w-[1100px] px-6 py-10">
      ...
      {setup && id && (
        <div className="mb-6 border border-neutral-300 p-3 text-sm">
          ...
          <div className="mt-2 flex items-center gap-2">
            <code className="block flex-1 break-all rounded bg-neutral-100 px-2 py-1 text-xs">
              https://haulhq.shop/setup/{setup}
            </code>
            <CopyButton text={`https://haulhq.shop/setup/${setup}`} />
          </div>
        </div>
      )}
      ...
      <section className="mb-12">
        <h2 className="mb-4 text-xs font-semibold uppercase tracking-tight">
          Friends &amp; their hauls
        </h2>
        {friends.length === 0 ? (
              ...
              {friends.map((f) => (
                <tr key={f.id} className="border-b border-neutral-100">
                  ...
                  <td className="py-2">
                    ...
                    {f.is_admin ? (
                      <span className="text-[10px] uppercase tracking-tight text-neutral-300">
                        You
                      </span>
                    ) : (
      ...
      <section className="mb-12">
        ...
        <form action={createFriend} className="flex flex-wrap items-center gap-3">
          ...
          <button
            type="submit"
            className="rounded bg-neutral-900 px-4 py-2 text-xs font-semibold uppercase tracking-tight text-white"
          >
            Add friend
          </button>
        </form>
      ...
      <section>
        ...
        <Link
          href="/admin/products"
          className="inline-block text-xs uppercase tracking-tight underline"
        >
          Manage products →
        </Link>
      </section>
