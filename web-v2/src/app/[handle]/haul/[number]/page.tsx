...
// Read-only view of an approved (past) haul. Same auth rule as the live haul
// page: the friend themselves, or an admin session.
export default async function PastHaulPage({
  params,
}: {
  params: Promise<{ handle: string; number: string }>;
}) {
  const { handle, number: numberParam } = await params;
  ...
  if (open?.haul.number === number) redirect(`/${handle}/haul`);
